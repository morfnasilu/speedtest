...
- (instancetype)initPrivate {
    if (self = [super init]) {
        self.username = @"";
        self.password = @"";
        self.progressAction = ^(NSInteger chunkSize, NSInteger totalSize, NSInteger finishedSize, CGFloat finishedPercent) {};
...
- (void)setLocalData:(NSData *)localData {
    if (![_localData isEqualToData:localData]) {
    ...
    switch (type) {
        ...
        case kCFStreamEventHasBytesAvailable: {
            ...
            if (bytesRead > 0) {
                ...
                request.progressAction(bytesRead, 0, (NSInteger)request.listData.length, 0);
            } else if (bytesRead == 0) {
                ...
                do {
                    ...
                    if (bytesParsed > 0) {
                        ...
                        request.progressAction(bytesParsed, 0, (NSInteger)totalBytesParsed, 0);
            ...
            if (bytesRead > 0) {
                ...
                do {
                    ...
                    if (bytesWritten > 0) {
                        ...
                        request.progressAction(bytesWritten, request.totalSize, request.finishedSize, (CGFloat)request.finishedSize / (CGFloat)request.totalSize * 100);
...
- (void)setLocalData:(NSData *)localData {
    ...
    self.totalSize = [super localData].length;
}
...
- (BOOL)start {
    if (self.localData) {
        self.readStream = CFReadStreamCreateWithBytesNoCopy(kCFAllocatorDefault, [self.localData bytes], self.localData.length, kCFAllocatorNull);
